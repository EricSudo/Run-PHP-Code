...
var View_Model = function() {
	...
	self.clear = function() {
		var answer = confirm("آیا مطمئن هستید که میخواهید کد ها را پاک کنید؟");
	...
	self.remote_import = function() {
		...
		var code_url = prompt('همیشه قبل از اجرای کد وارد شده از امنیت آن اطمینان حاص کنید!!!\n\nسرویس هایی که پشتیبانی میشود: \ngist.GitHub.com\nPasteBin.com\nPastie.org\n\nآدرس را وارد کنید:');
		if (code_url === null || code_url === '') return;
		code_id = get_id_from_url(code_url);
		self.editor.setValue('در حال اجرای کد...');
	...
	self.load_contributors = function() {
		if (self.contributors_loaded === false) {
			...
			$.get('https://api.github.com/repos/EricSudo/Run-PHP-Code/contributors', function(data) {
	...
	self.editor.on('change', function() {
		if (window.onbeforeunload == null) {
			window.onbeforeunload = function() { return "شما در ویرایشگر خود تغییراتی ایجاد کرده اید."; };
		}
	});
